...
* **Resource**: `ad_group_membership`: members named by SAM account name, distinguished name, common name or SID no longer produce a diff on every plan. The schema documents all of these as interchangeable with a GUID, but members were compared by GUID alone and the read wrote GUIDs back to state unconditionally — so any configuration not written in GUIDs disagreed with its own state forever, and Terraform proposed removing and re-adding every member. Comparison now considers every identifier form the directory returns, case-insensitively, and the read keeps the spelling the configuration uses. Members added outside Terraform still appear as their GUID, so drift is still reported.
