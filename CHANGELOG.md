...
SECURITY:
* **Provider**: Active Directory account passwords are no longer written to the provider log. `SanitiseString` logged both its input and its escaped output at DEBUG level, and every password set through `ad_user` passes through it, so `TF_LOG=DEBUG` or `TF_LOG_PROVIDER=DEBUG` — routine in CI — recorded the password twice in cleartext. This is the surface `initial_password_wo` does not cover: a write-only value is kept out of state and out of the plan file, and was written to the log regardless. ([#38](https://github.com/netresearch/terraform-provider-ad/pull/38))
* **Resource**: `ad_user`: a password containing a double quote is no longer printed when a command fails. The redaction patterns anchor on `-AsPlainText "…" -Force)`; such a password renders with an escaped quote inside that anchor, so no pattern matched and the full password reached the Terraform console and the CI log on every failed `New-ADUser` or `Set-ADAccountPassword`. The call site that builds the command now registers the exact text it embedded, and that text is replaced literally, which is insensitive to quoting and escaping. ([#38](https://github.com/netresearch/terraform-provider-ad/pull/38))
* **Provider**: the debug log for a failed command redacts the same text the error does. It ran before the CLIXML response was decoded, where a password containing an ampersand appears XML-encoded as `&amp;` and no literal replacement matches it. ([#38](https://github.com/netresearch/terraform-provider-ad/pull/38))

BUGFIXES:
* **Resource**: `ad_user`, `ad_group`, `ad_gpo`, `ad_gplink`, `ad_gpo_security`: destroying an object that is already gone succeeds again. v0.5.1 started checking the exit code of destructive commands, and recognised "the object is not there any more" only when Active Directory reported it as a transport error. The directory usually reports it as a non-zero exit code with the exception text on stderr, so a `terraform destroy` of an object removed out of band failed, and failed again on every retry. Both forms are recognised now. ([#38](https://github.com/netresearch/terraform-provider-ad/pull/38))
* **Provider**: an error message from Active Directory that is not valid CLIXML is kept instead of discarded. `decodeXMLCli` returned the empty string for a document it could not parse — a truncated response, for instance — while the caller logged that it was passing the text back unchanged, so the failure arrived with no explanation at all. ([#38](https://github.com/netresearch/terraform-provider-ad/pull/38))
* **Provider**: an empty element in a CLIXML response no longer panics the provider. The decoder indexed the first byte of each fragment before checking that the fragment had one. ([#38](https://github.com/netresearch/terraform-provider-ad/pull/38))
* **Resource**: `ad_gpo_security`: a refused removal of the security settings file reports the reason instead of `%!s(<nil>)`. ([#38](https://github.com/netresearch/terraform-provider-ad/pull/38))

NOTES:
* **Provider**: error messages from failed PowerShell commands are worded differently. They were not consistent before — five sites reported "while renaming GPO" for operations that rename nothing, including the SYSVOL file upload; two `ad_ou` messages named `Set-ADObject` for a `Rename-ADObject` command; retrieving a GPO's path reported a failure to retrieve the SYSVOL path. Every message now reads `while <what>: exit code <n>, stderr: …, stdout: …`. Configurations are unaffected; automation that matches on the old text is not. ([#38](https://github.com/netresearch/terraform-provider-ad/pull/38))
* **Provider**: a password can still be exposed through the command echo PowerShell puts in an error, when that echo arrives truncated inside the password or reassembled from CLIXML fragments across a line break. Redaction cannot reach text that was transformed before the provider received it. Suppressing the echo would remove the vector and also remove the command from every error message, which is a trade-off this release does not make. ([#38](https://github.com/netresearch/terraform-provider-ad/pull/38))

## v0.5.1 (netresearch fork, September 12, 2026)

BUGFIXES:
* **Resource**: `ad_user`, `ad_gpo`: a destroy that Active Directory refuses is an error again. `DeleteUser` and `DeleteGPO` discarded the PowerShell result and never checked its exit code, so a `Remove-ADUser` or `Remove-GPO` rejected by the directory — an account protected from accidental deletion, insufficient permissions — reported success, and Terraform dropped the resource from state while the object still existed. `ad_gpo` was affected the same way and had not been documented as such. ([#35](https://github.com/netresearch/terraform-provider-ad/pull/35))
